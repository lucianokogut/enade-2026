# 📘 ENADE 2026 – CST em Análise e Desenvolvimento de Sistemas (ADS)

## ✅ 1. Datas Limites (Resumo Objetivo)

### 📌 Concluintes
- **Período de inscrição:** 27/04/2026 a 18/05/2026
- **Retificação:** 19/05/2026 a 27/05/2026
- **Data limite final:** **27/05/2026**

#### Datas para cadastro dos concluintes no Sistema Enade
- Inscrição (cadastro) dos concluintes:
  - **27/04/2026 a 18/05/2026**
- Retificação das inscrições:
  - **19/05/2026 a 27/05/2026**

👉 Portanto, a data limite efetiva de cadastro (com retificação) é:
***27/05/2026***

---

### 📌 Ingressantes
- **Período de inscrição:** 17/08/2026 a 17/09/2026
- **Retificação:** 18/09/2026 a 30/09/2026
- **Data limite final:** **30/09/2026**

#### Datas para cadastro dos ingressantes no Sistema Enade
- Inscrição (cadastro) dos ingressantes:
  - **17/08/2026 a 17/09/2026**
- Retificação das inscrições:
  - **18/09/2026 a 30/09/2026**

👉 Portanto, a data limite efetiva de cadastro (considerando retificação) é:
***30/09/2026***

#### 🔴 Mudança na operacionalização do ingressante

##### 1. O que diz o Edital 2026 (item 3.1.4)

O edital estabelece que a regularidade do ingressante será atribuída mediante o preenchimento do Questionário do Ingressante.

👉 Condição única de regularidade (2026):
- Preencher o Questionário do Ingressante dentro do prazo

E reforça:
- Ingressante não faz prova
- Regularidade depende ativamente do estudante

##### 2. Como era no ciclo anterior (padrão histórico do ENADE)

Com base na regulamentação anterior (especialmente Inep via Portaria Normativa nº 840/2018 e replicações até 2023/2024), a regra prática era:

- O ingressante era inscrito pela IES
- O ingressante era dispensado da prova
- A regularidade era automática, desde que estivesse devidamente inscrito

Em muitos ciclos, o Questionário do Ingressante existia, mas:
- Tinha caráter mais diagnóstico
- Não era condição efetiva de regularidade com o mesmo peso operacional atual

##### 3. Mudança prática (o que realmente mudou)

🔴 **Antes (ciclos anteriores)**
- Regularidade = ato da IES
- Foco: inscrição correta
- Estudante: papel passivo

🟢 **Agora (ENADE 2026)**
- Regularidade = ato do estudante
- Foco: preenchimento do questionário
- Estudante: papel ativo e obrigatório

##### 4. Interpretação técnica da mudança

A mudança não altera o conceito de dispensa da prova, mas altera:

- **Natureza da regularidade**
  - Antes: administrativa (IES)
  - Agora: procedimental + individual (estudante)
- **Responsabilização**
  - Transferência parcial de responsabilidade da IES para o estudante
- **Risco operacional**
  - Aumenta significativamente, porque o aluno pode ser inscrito corretamente e ainda assim ficar irregular

##### 5. Impacto prático para a coordenação

Essa mudança cria um novo ponto crítico:

👉 Ingresso ≠ regularidade garantida

Agora é necessário:
- Monitorar preenchimento do questionário
- Cobrar ativamente os ingressantes
- Registrar evidências de acompanhamento

##### 6. Síntese objetiva para os Ingressantes

| Aspecto      | Ciclos anteriores            | ENADE 2026               |
| ------------ | ---------------------------- | ------------------------ |
| Prova        | Não realiza                  | Não realiza              |
| Inscrição    | Obrigatória (IES)            | Obrigatória (IES)        |
| Questionário | Secundário                   | Obrigatório              |
| Regularidade | Automática após inscrição    | Depende do estudante     |
| Risco        | Baixo                        | Alto                     |

***TURMA 2026/1*** **torna-se obrigada a preencher o Questionário até 17/09/2026**, sem considerar a data de retificação

---

### 📌 Colação de Grau sem necessidade de ENADE
- **Data limite:** **31/08/2026**

**Regra:**
- Colação até 31/08/2026 → dispensa do ENADE
- Colação após 31/08/2026 → obrigatoriedade de participação no ENADE

#### Data limite para colação de grau sem precisar fazer o ENADE
Isso vem do critério de habilitação dos concluintes:

- Para CST:
  São concluintes aqueles que NÃO tenham previsão de colação de grau até 31 de agosto de 2026
  👉 Interpretação direta:
    - Se o estudante colar grau até 31/08/2026 → NÃO precisa fazer o ENADE
    - Se a colação for após 31/08/2026 → será considerado concluinte e deve fazer o ENADE

👉 Portanto, a data limite é:
***31/08/2026***

---

### ✅ Resumo final (ADS / CST)

| Situação                     | Data limite        |
| ---------------------------- | ------------------ |
| Cadastro de **concluintes**  | **27/05/2026**     |
| Cadastro de **ingressantes** | **30/09/2026**     |
| Colação de grau sem ENADE    | **até 31/08/2026** |

---

## 🧭 2. Checklist Operacional – Coordenação de Curso

---

### 🔹 FASE 1 — Preparação Inicial
📅 27/04 a 18/05/2026

- [ ] Acessar o Sistema Enade
- [ ] Conferir dados do curso no e-MEC
- [ ] Validar enquadramento do curso (ADS)
- [ ] Garantir acesso do coordenador e PI
- [ ] Levantar lista preliminar de alunos
  - [ ] Possíveis concluintes
  - [ ] Alunos próximos da integralização

⚠️ **Risco:** dados incorretos ou alunos não identificados

---

### 🔹 FASE 2 — Cadastro dos Concluintes (CRÍTICA)
📅 27/04 a 18/05/2026  
🔁 Retificação: 19/05 a 27/05/2026

- [ ] Identificar concluintes (≥75% carga horária e sem colação até 31/08/2026)
- [ ] Inserir TODOS no Sistema Enade
- [ ] Conferir dados obrigatórios
  - [ ] Percentual de integralização
  - [ ] Previsão de formatura
- [ ] Revisar lista completa
- [ ] Fazer auditoria interna (planilha de controle)

📌 **Prazo final:** 27/05/2026

⚠️ **Riscos críticos:**
- Omissão de aluno → irregularidade
- Inclusão indevida → inconsistência institucional

---

### 🔹 FASE 3 — Acompanhamento dos Concluintes
📅 01/06 a 29/11/2026

- [ ] Orientar alunos a preencher:
  - [ ] Cadastro no sistema
  - [ ] Questionário do Estudante
- [ ] Monitorar participação continuamente
- [ ] Apoiar demandas (ex: atendimento especial)

⚠️ **Risco:** aluno não responde questionário → fica irregular

---

### 🔹 FASE 4 — Ponto de Corte para Dispensa do ENADE
📅 Até 31/08/2026

- [ ] Identificar alunos aptos à colação antecipada
- [ ] Validar integralização total
- [ ] Organizar colação de grau

📌 **Regra-chave:**
- Até 31/08/2026 → dispensado
- Após 31/08/2026 → obrigatório fazer ENADE

⚠️ **Risco crítico:** erro na previsão de formatura

---

### 🔹 FASE 5 — Cadastro dos Ingressantes
📅 17/08 a 17/09/2026  
🔁 Retificação: 18/09 a 30/09/2026

- [ ] Identificar ingressantes (0–25% carga horária)
- [ ] Inserir todos no sistema
- [ ] Conferir dados cadastrais

📌 **Prazo final:** 30/09/2026

⚠️ **Observação:** ingressantes não fazem prova

---

### 🔹 FASE 6 — Prova do ENADE
📅 29/11/2026

- [ ] Reforçar comunicação com alunos
- [ ] Confirmar local de prova
- [ ] Garantir comparecimento

---

### 🔹 FASE 7 — Pós-Prova e Regularidade
📅 03/12/2026 a 11/01/2027

- [ ] Registrar presença
- [ ] Conferir situação regular
- [ ] Tratar dispensas, se necessário

⚠️ **Regra crítica:**
- Sem regularidade → impedimento de colação de grau

---

## 🚨 3. Pontos Críticos (Resumo Rápido)

- **27/05/2026** → fim do cadastro de concluintes
- **31/08/2026** → limite para colar grau sem ENADE
- **30/09/2026** → fim do cadastro de ingressantes
- **29/11/2026** → prova do ENADE

---

## 💡 4. Boas Práticas Recomendadas

- Manter planilha de controle paralela
- Realizar dupla conferência antes dos prazos críticos
- Comunicar alunos com antecedência
- Registrar todas as ações para auditoria

---
